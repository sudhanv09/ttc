import lib/[magnet_parser, trackers, peer_manager]
...
proc hexToByteArray(hex: string): array[20, byte] = 
    for i in 0..<20:
        let start = i * 2
        if start + 1 < hex.len:
            result[i] = byte(parseHexInt(hex[start..start+1]))

proc strToByte(str: string): array[20, byte] = 
    for i in 0..<min(str.len, 20):
        result[i] = byte(str[i])
...
proc sendRequestsToTrackers(trackers: seq[string], req: TrackerRequest) {.async.} =
    ...
    for i, response in responses:
        if response != "":
            ...
            let handshakeObj = initHandshake(HandShake(infoHash: req.infoHash.hexToByteArray, peerId: req.peerId.strToByte))
            echo verifyMessage(handshakeObj, req.infoHash)
